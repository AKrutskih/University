    ...
    for iterOfMap in range (1):
        ...
        for robot in numbersOfRobots:
            for iterOfRobots in range(1):
                ...
                pl = plt.imshow(myMap, cmap = plt.get_cmap("BrBG_r") )
                fig.colorbar(pl)
                ...
                for i in range (robot):
                    ...
                    ax.plot ([x for x, y in samePath] , [y for x, y in samePath], linewidth=1.5,  color = 'k')
                    ax.plot(samePath[0][0], samePath[0][1], "ro")
                    ax.plot(samePath[len(samePath)-1][0], samePath[len(samePath)-1][1], "ro", color = 'b')
                    fig.savefig('fig/' + str(DIM) + 'rob' + str(robot) + '.png')
                    plt.close(fig)
